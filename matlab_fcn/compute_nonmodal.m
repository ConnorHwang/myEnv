function [M_growth, output1, output2] = ...
    compute_nonmodal(We, n, r, N1, L1, N2, L2, Eigenvectors, Eigenvalues, K, t, normtype)
%--------------------------------------------------------------------------
...
% terms.
Q0([P2vec V2_vec W2_vec U2_vec P1vec V1_vec W1_vec U1_vec],:) = [];
...
[~, m2] = MJet(N2,L1,L1+L2);
m2 = m2 * sqrt(r);
mint = sqrt(1/We*(-1+n^2));
mint_= sqrt(1/We);
c1 = sqrt(1/2);
% mint = sqrt(B);
% Constraints to the all space.
CONSTRAINT = blkdiag(c1*m2,c1*m2,c1*m2,mint,mint_,c1*m1,c1*m1,c1*m1);
% Customized energy Matrix
IEN = blkdiag(zeros(N2), zeros(N2), zeros(N2),mint,mint_,zeros(N1), zeros(N1), zeros(N1));
% Connor's method,
...
% Determine outputs
iniTotalEnergy = CONSTRAINT*(Q0*Kappa);
% finIntfEnergy = IEN*(Q1*Kappa);
finIntfEnergy = CONSTRAINT*(Q1*Kappa);
output1 = Q0*Kappa;
output2 = Q1*Kappa;
end