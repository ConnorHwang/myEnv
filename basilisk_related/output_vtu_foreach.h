/*
 * This function writes one XML file which allows to read the *.vtu files
 * generated
 * by output_vtu_ascii_foreach() when used in MPI. Tested in (quad- and
 * oct-)trees
 * using MPI.
 * */
...
/*
 * This function writes one XML VTK file per PID process of type unstructured
 * grid
 * (*.vtu) which can be read using Paraview. File stores scalar and vector
 * fields
 ...
 * output_pvtu_ascii() above to read in parallel. Tested in (quad- and
 * oct-)trees
