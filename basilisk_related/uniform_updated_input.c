...
#define omega  2.2/0.0039
#define tzero  .09
// n5low: 0.201671; n5high: 0.655438; n6: 0.778920
#define Famp   radius * 4 * 0.201671
#define Nazm   5.
/**
Output for vtk files */
int    Nx      = 64*nx_proc; 
double Nlen    = 0.016; // about 30 points within the radius
/** boundary conditions */
...
event init (t = 0) {
  if (!restore (file = "dump")) {
    ...
    FILE * fpb = fopen("./newinput/n5low_base.txt","r");
    if (fpb == NULL) {
    ...
    FILE * fpu = fopen("./newinput/n5low_ur.txt","r");
    if (fpu == NULL) {
      fprintf(stderr, "Could not find the inputfile!\n");
      return 1;
    }
    ...
    fclose(fpu);
    fpu = fopen("./newinput/n5low_ui.txt","r");
    if (fpu == NULL) {
      fprintf(stderr, "Could not find the inputfile!\n");
      return 1;
    }
    ...
    FILE * fpv = fopen("./newinput/n5low_vr.txt","r");
    if (fpv == NULL) {
      fprintf(stderr, "Could not find the inputfile!\n");
      return 1;
    }
    ...
    fclose(fpv);
    fpv = fopen("./newinput/n5low_vi.txt","r");
    if (fpv == NULL) {
      fprintf(stderr, "Could not find the inputfile!\n");
      return 1;
    }
    ...
    FILE * fpw = fopen("./newinput/n5low_wr.txt","r");
    if (fpw == NULL) {
      fprintf(stderr, "Could not find the inputfile!\n");
      return 1;
    }
    ...
    fclose(fpw); 
    fpw = fopen("./newinput/n5low_wi.txt","r");
    if (fpw == NULL) {
      fprintf(stderr, "Could not find the inputfile!\n");
      return 1;
    }
